...
class EmailService:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.warning("resend 模块未安装，邮件发送功能不可用")

    def _create_verification_email_html(self, code: str, purpose: str) -> str:
        """创建验证邮件HTML内容"""
...
def send_verification_email(self, email: str, purpose: str) -> dict:
    ...
    try:
        if not email_service.available:
            raise Exception("resend 模块未安装，无法发送邮件")
        ...
        # 发送邮件
        result = email_service.resend.Emails.send(params)
